 ...
 *
 * This file contains a thread for data sharing between two connected devices
 */
class ConnectedThreadWithRequestCodes extends Thread {
    ...
    String ratingPlusDevice=new String();
    String Role=new String();
    //Constructore which takes a socket and a context object
    ...
    public void run() {
        ...
        long curTime=0;
        int flagForDone=0;
        int flagForPostCOnnOnce=0;
        while (true) {
            /*New Code*/
            if(curTime==0)
            curTime=System.currentTimeMillis();
            //This condition checks if no message has been received from other device in the last 7 seconds, if it is not,
            //I myself start transferring any messages that I might have
            if(flagForDone==1 && System.currentTimeMillis()-curTime>7000 )
            {
                new DbFunctions().unsetConnectedLogTBL(ConstantsClass.mydatabaseLatest,mmSocket.getRemoteDevice().getAddress());
                break;
            }
            if(ifIGotMessageTsrs==1 && System.currentTimeMillis()-curTime>7000 && flagForPostCOnnOnce==0) {
                postConnMTransfer();
                flagForPostCOnnOnce++;
                curTime=System.currentTimeMillis();
                flagForDone=1;
                ...
                if (new String(preamble).split("::")[0].equals("Preamble")) {
                    ...
                    if(MessageType!=Constants.MESSAGE_IMAGE_ACK && MessageType!=Constants.MESSAGE_IMAGE_BACK_ACK && MessageType!=Constants.MESSAGE_INCENT_REQ && MessageType!=Constants.MESSAGE_INCENT_REW && MessageType!=Constants.MESSAGE_INCENT_REP && MessageType!=Constants.MESSAGE_TRANS_DONE) {
                        ...
                        if (new String(preamble).contains("Lengths")) {
                            ...
                            ratingPlusDevice=new String(preamble).split("::")[13];
                        }
                    }
                    ...
                    switch (MessageType) {
                        //If a message with Message type MESSAGE_IMAGE IS RECEIVED
                        case Constants.MESSAGE_IMAGE:
                            try {
                                ...
                                int noOfBytes=4192;
                                if (MessageSize != 0) {
                                    //Read bytes from remote device for the image
                                ...
                                //Saving message ratings
                                if(ratingPlusDevice.length()>0)
                                {
                                    String MacRate[]=ratingPlusDevice.split(">");
                                    for(String macRate:MacRate)
                                    {
                                        String mac=macRate.split("<")[1];
                                        double rate=Double.parseDouble(macRate.split("<")[1]);
                                        ConstantsClass.mydatabaseLatest.execSQL("INSERT OR IGNORE INTO UUID_DEVICE_RATE_TBL VALUES('"+UUIDReceived+"','"+mac+"',"+rate+")");
                                    }
                                }

                                Cursor ifMessageAlreadyExists=ConstantsClass.mydatabaseLatest.rawQuery("SELECT * from MESSAGE_TBL where UUID='"+UUIDReceived+"'",null);
                                if(ifMessageAlreadyExists.getCount()==0)
                                dbFunctions.insertIntoMsgTblRemoteMsg(ConstantsClass.mydatabaseLatest,deviceNameReceived,macAddrReceived,tagsReceived,latitudeReceived,longitudeReceived,timestampReceived,android.provider.Settings.Secure.getString(context.getContentResolver(), "bluetooth_address"),BluetoothAdapter.getDefaultAdapter().getName(),mimeReceived,formatRecived,Environment.getExternalStorageDirectory().getAbsoluteFile()+"/"+fileNameImage,UUIDReceived,sizeReceived,qualityReceived,priorityReceived);
                                new DbFunctions().insertIntoSentLogTBL(ConstantsClass.mydatabaseLatest,UUIDReceived,mmSocket.getRemoteDevice().getAddress(),1);
                                //Give incentive if I am the destination

                                int amIDest=0;
                                Cursor findUUIDInc=ConstantsClass.mydatabaseLatest.rawQuery("SELECT incentive,flag from INCENT_UUID_MAC_TBL WHERE UUID='"+ UUIDReceived+"' and MacAd='"+mmSocket.getRemoteDevice().getAddress()+"'",null);
                                ...
                                //See if I am the destination or relay for the message
                                ...
                                //This means that I am the relay with average value of weights of keywords
                                 ...
                                 if(amIDest==1) {
                                    ...
                                    ConstantsClass.mydatabaseLatest.execSQL("UPDATE INCENTIVES_TBL SET incentive=incentive-" + incentive);
                                     Log.d("CTwRC","Incentive before deletion in INCENTIVES_TBL:"+incentive);
                                     ConstantsClass.mydatabaseLatest.execSQL("INSERT INTO LAST_FIVE_TRANS VALUES('"+mmSocket.getRemoteDevice().getAddress()+"','"+UUIDReceived+"',"+incentive+",0.0,(datetime('now','localtime')))");
                                     new DbFunctions().deleteLeastRecent(ConstantsClass.mydatabaseLatest);
                                ...
                                {
                                        ConstantsClass.mydatabaseLatest.execSQL("INSERT OR IGNORE INTO INCENT_FOR_MSG_TBL VALUES('" + UUIDReceived + "',"+Double.parseDouble(df.format(incentive))+",0.0,0.0)" );
                        ...
                        case Constants.MESSAGE_INCENT_REW:
                            //This is the case for receiving the incentive reward
                            DecimalFormat df=new DecimalFormat("#.####");
                            String UUID=new String(preamble).split("::")[2];
                            double inc=Double.parseDouble(new String(preamble).split("::")[3]);
                            Cursor cursorForMesInc=ConstantsClass.mydatabaseLatest.rawQuery("SELECT * from INCENT_FOR_MSG_TBL where UUID='"+UUID+"'",null);
                            if(cursorForMesInc.getCount()>0) {
                                ConstantsClass.mydatabaseLatest.execSQL("UPDATE INCENT_FOR_MSG_TBL set received=received+" + Double.parseDouble(df.format(inc)) + " where UUID='" + UUID + "'");
                                ConstantsClass.mydatabaseLatest.execSQL("UPDATE INCENTIVES_TBL SET incentive=incentive+"+Double.parseDouble(df.format(inc)));
                                ConstantsClass.mydatabaseLatest.execSQL("INSERT INTO LAST_FIVE_TRANS VALUES('"+mmSocket.getRemoteDevice().getAddress()+"','"+UUIDReceived+"',0.0,"+inc+",(datetime('now','localtime')))");
                                new DbFunctions().deleteLeastRecent(ConstantsClass.mydatabaseLatest);
                            }
                        ...
                        case Constants.MESSAGE_TSRS:
                            ...
                            for (String curString : ratingStringAr) {
                                ...
                                if (!MacAd.equals(android.provider.Settings.Secure.getString(context.getContentResolver(), "bluetooth_address"))) {
                                    ...
                                    Cursor noOfEntries = ConstantsClass.mydatabaseLatest.rawQuery("SELECT * from USER_RATING_MAP_TBL where MacAd='" + MacAd + "'", null);
                                    if (noOfEntries.getCount() > 0) {
                                        ...
                                        ConstantsClass.mydatabaseLatest.execSQL("UPDATE USER_RATING_MAP_TBL set rating=" + rating + " WHERE MacAd='" + MacAd + "'");
                                    } else {
                                        rating = 4 + rating * 0.2;
                                        ConstantsClass.mydatabaseLatest.execSQL("INSERT INTO USER_RATING_MAP_TBL VALUES('" + MacAd + "'," + rating + ",'" + mmSocket.getRemoteDevice().getAddress() + "')");
                                    }
                                }
                            }
                        }
                            ...
                            Cursor cursorForRole=ConstantsClass.mydatabaseLatest.rawQuery("SELECT Role from ROLE_TBL where MACAd='"+mmSocket.getRemoteDevice().getAddress()+"'",null);
                            if(cursorForRole.getCount()==0)
                            {
                                ConstantsClass.mydatabaseLatest.execSQL("INSERT OR IGNORE INTO ROLE_TBL VALUES("+Role+",'"+mmSocket.getRemoteDevice().getAddress()+"')");
                            }
                            else
                            {
                                ConstantsClass.mydatabaseLatest.execSQL("UPDATE ROLE_TBL SET Role="+Role+" WHERE MACAd='"+mmSocket.getRemoteDevice().getAddress()+"'");
                            }

                            //Find role of self
                            String Role=new String();
                            Cursor cursorForRoleSelf=ConstantsClass.mydatabaseLatest.rawQuery("SELECT Role from ROLE_TBL where MACAd='SELF'",null);
                        ...
                        case Constants.MESSAGE_TSRS_BACK:
                        {
                            ...
                            ConstantsClass.mydatabaseLatest.execSQL("DELETE FROM TSR_SHARE_DONE_TBL WHERE MacAd='"+mmSocket.getRemoteDevice().getAddress()+"'");
                            Cursor cursorForTSRShareDone=ConstantsClass.mydatabaseLatest.rawQuery("SELECT * from TSR_SHARE_DONE_TBL",null);
                            if(cursorForTSRShareDone.getCount()==0)
                            {
                                postConnMTransfer();
                            }
                            //new DbFunctions().sendMessage(ConstantsClass.mydatabaseLatest,mmSocket.getRemoteDevice().getAddress(),mmSocket.getRemoteDevice().getName(),this,Constants.MESSAGE_IMAGE);
                        }
                            break;
                        ...
                        case Constants.MESSAGE_IMAGE_BACK:
                            try {
                                ...
                                Cursor cursorForMac=ConstantsClass.mydatabaseLatest.rawQuery("SELECT sourceMacAddr from MESSAGE_TBL where UUID='"+UUIDReceived+"'",null);
                                String sourceMac="";
                                ...
                                {
                                    new DbFunctions().insertInc(ConstantsClass.mydatabaseLatest,-1.0);
                                }
                                else
                                {
                                    new DbFunctions().insertInc(ConstantsClass.mydatabaseLatest,-0.5);
                                }
                        ...
                        case Constants.MESSAGE_IMAGES_TOTAL:
                        {
                            ...
                            if(numImages==0)
                            {
                                new DbFunctions().sendMessage(ConstantsClass.mydatabaseLatest, mmSocket.getRemoteDevice().getAddress(), mmSocket.getRemoteDevice().getName(), this, Constants.MESSAGE_IMAGE_BACK);
                            }
                        }
                        break;
                        ...
                        case Constants.MESSAGE_IMAGE_BACK_ACK:
                        {
                            ...
                            Cursor cursorForMac=ConstantsClass.mydatabaseLatest.rawQuery("SELECT sourceMacAddr from MESSAGE_TBL where UUID='"+UUID+"'",null);
                            String sourceMac="";
                        ...
                        case Constants.MESSAGE_INCENT_REQ:
                            ...
                            Cursor noOfEntries = ConstantsClass.mydatabaseLatest.rawQuery("SELECT * from USER_RATING_MAP_TBL where MacAd='" + sourceMac + "'", null);
                            double rating=5.0;
                            ...
                            if(new String(preamble).contains("addedTags")) {
                                String someaddedTags = new String(preamble).split("::")[4].split("\\)\\)")[1];
                                String addedTagsMac[]=someaddedTags.split("\\+\\+");
                                for(int someC=0;someC<addedTagsMac.length;someC++)
                                {
                                    addedTags+=addedTagsMac[someC].split("--")[0];
                                    ConstantsClass.mydatabaseLatest.execSQL("INSERT OR IGNORE INTO ADDED_TAGS_TBL VALUES('"+UUID+"','"+addedTagsMac[someC].split("--")[0]+"','"
                                            +addedTagsMac[someC].split("--")[1]+"',5.0)");
                                }
                                ...
                                if (addedTags != null && addedTags.length() > 0) {
                                    ...
                                    for (int adI = 0; adI < addedTagsSplit.length; adI++) {
                                        selfTags = ConstantsClass.mydatabaseLatest.rawQuery("SELECT belongs_to from TSR_TBL where SI='" + addedTagsSplit[adI] + "' and belongs_to='SELF'", null);
                            ...
                            Cursor cursorIncent=ConstantsClass.mydatabaseLatest.rawQuery("SELECT incentive from INCENTIVES_TBL",null);
                            double sumIncents=0.0;
                            while(cursorIncent.moveToNext())
                            {
                                incentivePresent=cursorIncent.getDouble(0);
                                Cursor cursorForReqsCurIt=ConstantsClass.mydatabaseLatest.rawQuery("SELECT SUM(incentive) FROM INCENT_UUID_MAC_TBL",null);
                                ...
                                if(flagRec==1)
                                ConstantsClass.mydatabaseLatest.execSQL("INSERT OR IGNORE INTO INCENT_UUID_MAC_TBL VALUES('"+mmSocket.getRemoteDevice().getAddress()+"','"+UUID+"',"+incentiveRequired+",1)");
                                else
                                    ConstantsClass.mydatabaseLatest.execSQL("INSERT OR IGNORE INTO INCENT_UUID_MAC_TBL VALUES('"+mmSocket.getRemoteDevice().getAddress()+"','"+UUID+"',"+incentiveRequired+",0)");
                            }
                            ...
                            Cursor ifMessageAlreadyExists=ConstantsClass.mydatabaseLatest.rawQuery("SELECT * from MESSAGE_TBL where UUID='"+UUID+"'",null);
                            if(ifMessageAlreadyExists.getCount()>0) flag=0;

                            if(flag==1) {
                                Cursor testTBLRows = ConstantsClass.mydatabaseLatest.rawQuery("SELECT * from INCENT_UUID_MAC_TBL", null);
    ...
    public void sendMessage(String UUIDRep,int isTherePromise,double incentive)
    {
        try {
            ...
            Cursor cursorForNewTags=ConstantsClass.mydatabaseLatest.rawQuery("SELECT * from ADDED_TAGS_TBL where UUID='"+UUIDRep+"'",null);
            String addedTags="";
            String ratingPlusDevice="";
            while(cursorForNewTags.moveToNext())
            {
                addedTags+=cursorForNewTags.getString(1);
                String mac=cursorForNewTags.getString(2);
                double rating=cursorForNewTags.getShort(3);
                ratingPlusDevice+=mac+"<"+rating+">";
            }
            if(ratingPlusDevice.length()!=0)
            ratingPlusDevice=ratingPlusDevice.substring(0,ratingPlusDevice.length()-2);
            if(!addedTags.equals(""))
            ...
            preambleString= "Preamble::MessageType:" + codeForMessage + "::MessageSize:" +
                    ...
                    mime + "::" + format + "::" + localMacAddr + "::" + localName + "::" + UUIDRep + "::" + size + "::" + quality + "::" + priority + "::"+ratingPlusDevice+"::";
            if(isTherePromise==1)
            {
